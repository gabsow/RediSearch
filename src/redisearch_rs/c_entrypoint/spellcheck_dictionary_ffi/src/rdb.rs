...
//! [`SpellCheckDictionary::rdb_load`], driven through the
//! `RedisModuleIO`-backed transport shared with the `trie_rdb_ffi` crate.
...
use trie_rdb_ffi::{RmIoReader, RmIoWriter};
...
pub unsafe extern "C" fn SpellCheckDictionary_RdbSave(
    ...
    // (1.), which bounds the writer's lifetime.
    let mut writer = unsafe { RmIoWriter::new(io) };
    dictionary.rdb_save(&mut writer);
}
...
pub unsafe extern "C" fn SpellCheckDictionary_RdbLoad(
    ...
    // Safety: caller guarantees `io` is valid for the duration of the call
    // (1.), which bounds the reader's lifetime.
    let mut reader = unsafe { RmIoReader::new(io) };
    match SpellCheckDictionary::rdb_load(&mut reader) {
