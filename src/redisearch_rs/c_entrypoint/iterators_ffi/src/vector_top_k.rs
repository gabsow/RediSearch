...
use rqe_iterators::{
    ExpirationChecker, FieldExpirationChecker, NoOpChecker,
    c2rust::CRQEIterator,
...
///    callback populated.
/// 5. `filter_ctx`, when non-null, is [valid] for a [`FieldFilterContext`] for
///    the duration of this call.
/// 6. `sctx`, when non-null, is [valid] for a [`RedisSearchCtx`] with a [valid]
...
pub unsafe extern "C" fn NewVectorTopKIterator(
    ...
    // Apply the per-field expiration checker when the caller supplies both
    // `sctx` and `filter_ctx`; otherwise disable expiration via `NoOpChecker`.
    // Each checker type monomorphizes a distinct iterator, both type-erased into
    // the returned `*mut QueryIterator`.
    match (NonNull::new(sctx), filter_ctx.is_null()) {
        (Some(sctx_nn), false) => {
...
/// reads. Generic over the expiration checker so both monomorphizations of
/// [`NewVectorTopKIterator`] share one boxing path.
fn box_reduced<'index, E: ExpirationChecker + 'index>(
    reduced: NewVectorTopK<'index, E>,
) -> *mut QueryIterator {
    let clear_skip_to = |ptr| unsafe { patch_vtable(ptr, |h| h.SkipTo = None) };
    match reduced {
