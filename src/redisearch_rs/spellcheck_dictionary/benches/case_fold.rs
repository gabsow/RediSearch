...
//! (so the match set — and thus the work past folding — is unchanged). The
...
//! copy-on-write (COW) allocation signal. Both query paths reach
//! `string_utils::unicode_tolower{,_capped}`, which *always* allocate a folded
//! `String`; there is no borrow-when-already-lowercase fast path to bypass. So
//! the lower/mixed delta here reflects `char::to_lowercase` mapping CPU only,
//! not an avoided allocation, and is expected to be small. Should a
//! borrowing `unicode_tolower_cow` ever be adopted here, this same bench would
//! begin to capture the real COW delta.
//!
//! Both paths also linearly scan and fold *every* stored term on every query
//! (`self.trie.iter()` in `contains`/`fuzzy_matches`), so the per-query cost is
//! dominated by `corpus.len()` folds regardless of case.
